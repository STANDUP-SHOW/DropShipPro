...
import { Eye, Download, Trash2, ExternalLink, ChevronRight, Loader2, LifeBuoy } from 'lucide-react'
...
/**
 * Les publicités réalisées, rangées par annonce, et le geste qui les diffuse.
 *
 * Extrait de la page « Mes pubs » le 19/09/2026, parce que la page Réseaux
 * demande la même liste avec le même bouton : deux copies auraient divergé au
 * premier correctif, et c'est le bouton « Diffuser » — celui qui engage une
 * campagne — qu'on ne veut pas voir se dédoubler.
 *
 * **Ce que « Diffuser » veut dire ici, et c'est important :** aucune régie ne
 * nous laisse déposer une créative sans compte publicitaire validé. Avec un
 * compte relié, la campagne part vraiment ; sans, le bouton télécharge le
 * fichier et ouvre le gestionnaire de la régie. Écrire « Publier » dans ce
 * second cas serait mentir, et un vendeur qui croit sa campagne partie ne la
 * surveille pas.
 */
...
export function ListePubs({
  /** Où relier un compte publicitaire, dit au vendeur quand rien n'est branché. */
  ouRelier = 'Réseaux',
}: {
  ouRelier?: string
}) {
                    ...
                    {groupe.pubs.map((img) => {
                      ...
                      return (
                        <li key={img.id} className="overflow-hidden rounded-xl border border-white/10 bg-black/20">
                          ...
                          >
                            <img src={assetUrl(img.path)} alt="" loading="lazy" className="w-full" />
                          </button>
                            ...
                            */}
                            {comptePour(img.platform) ? (
                              ...
                              <p className="mt-2 text-[11px] leading-relaxed text-gray-600">
                                {`« Diffuser » télécharge le fichier et ouvre ${gestionnaire.label} : aucun compte publicitaire n'est relié. Reliez-le depuis ${ouRelier} pour déposer directement.`}
                              </p>
                            ) : null}
                          </div>
                        </li>
